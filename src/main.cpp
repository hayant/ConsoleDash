...
static std::optional<int> parse_positive_int(const std::string& s) {
    try {
        size_t parsed = 0;
...
static void parse_level_parameters(const std::vector<std::string>& parameter_lines, LevelParameters& parameters) {
...
static bool load_level_from_file(const std::string& path, consoledash::ConsoleDash& game, LevelParameters& parameters) {
    using namespace consoledash;
...
int main(int argc, char** argv) {
    ...
    LevelParameters level_parameters;
    if (argc >= 2) {
        const std::string level_path = argv[1];
        if (!load_level_from_file(level_path, game, level_parameters)) {
            restore_input();
